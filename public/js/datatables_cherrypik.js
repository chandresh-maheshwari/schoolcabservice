            ...
            response = [
                {
                    ...
                    orderable: false,
                    render: function (data, type, row, meta) {
                        const checkbox = row.can_manage === false
                            ? ''
                            : `<input type="checkbox" class="multi-delete-checkbox" value="${row.id}">`;
                        return `
                            ${checkbox}
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        let actionBtn = "";
                        const canManageRow = row.can_manage !== false;
                        if (canManageRow && canModuleAction('update')) {
                            actionBtn += `
                        <label class="switch" title="${row.status ? 'Change Status to Inactive' : 'Change Status to Active'}">
                        ...
                        if (canManageRow && canModuleAction('edit')) {
                        ...
                        if (canManageRow && canModuleAction('destroy')) {
                            actionBtn += `<button class="btn btn-oblong btn-danger btn-sm" title="Delete" id="deleteCMSCategory" onclick="deleteData(this , '${tableId}' , '${deleteRoute}')" data-id="${row.id}"><i class="fa fa-trash"></i></button>`;
                        }
                        return actionBtn;
                    },
            ...
            response = [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        const badgeStyle = `display:inline-block;padding:4px 10px;border-radius:4px;background:${badgeColor};color:#fff;font-size:12px;font-weight:700;line-height:1;`;
