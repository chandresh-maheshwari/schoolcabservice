...
class MobileAuthController extends Controller
{
    public function login(Request $request): JsonResponse
    {
        ...
        $activeMatch = $this->resolveAuthUserByIdentifier($login, $requestedRole, $registeredEmail, false);
        if (! $activeMatch) {
        ...
        if (! $this->passwordMatches($password, (string) ($activeMatch['user']->password ?? ''))) {
    ...
    public function sendEmailOtp(Request $request): JsonResponse
    {
        ...
        Otp::updateOrCreate(['user_id' => $activeMatch['user']->id], ['otp' => $otp]);

        Mail::to($activeMatch['email'])->send(new MobileLoginOtpMail($otp, $activeMatch['role'], 'mobile-login'));
    ...
    public function verifyEmailOtp(Request $request): JsonResponse
    {
        ...
        $otpRecord = Otp::query()
            ->where('user_id', $activeMatch['user']->id)
            ->where('otp', $otp)
            ->first();
    ...
    public function forgotPassword(Request $request): JsonResponse
    {
        ...
        $activeMatch = $this->resolveAuthUserByIdentifier($email, null, $email, false);
        if (! $activeMatch) {
            return response()->json([
        ...
        $otp = $this->generateOtp();
        Otp::updateOrCreate(['user_id' => $activeMatch['user']->id], ['otp' => $otp]);
        Mail::to($activeMatch['email'])->send(new MobileLoginOtpMail($otp, $activeMatch['role'], 'forgot-password'));
    ...
    {
        $rolesToSearch = $requestedRole ? [$requestedRole] : ['driver', 'parent'];
