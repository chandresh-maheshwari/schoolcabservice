            ...
            <div class="container">
                <nav aria-label="breadcrumb-nav">
                    <ol class="breadcrumb breadcrumb-style-2 my-20">
                        ...
                        <li class="breadcrumb-item breadcrumb-item-style-2 active" aria-current="page">Update Teams</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                <h4 class="about-us-create-header">Update Teams Details</h4>
            </div>
            <div class="card-body">
                <form id="heroSectionForm" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="4" required>{{ $teams->description }}</textarea>
                    </div>
                   {{-- <div class="form-group">
                        ...
                        <div class="mt-2">
                            ...
                            <span id="imageName">{{ $teams->image ? basename($teams->image) : 'No image selected' }}</span>
                            <img id="imagePreview" src="{{ $teams->image ? asset($teams->image) : '' }}" alt="Image Preview"
                                style="display: {{ $teams->image ? 'block' : 'none' }}; width: 100px; height: 100px; margin-top: 10px;">
                            @if($teams->image)
                    ...
                    <div class="form-group">
                        ...
                        <div class="mt-2">
                            ...
                            <button type="button" class="btn btn-primary" id="uploadImageBtn"
                                ...
                                style="background-color: #2C9DD4; color: white;">Upload Image</button>
                            @php
                                $imagePath = $teams->image ? public_path($teams->image) : null;
                                $imageExists = $imagePath && File::exists($imagePath);
                                $imageUrl = $imageExists ? asset($teams->image) : asset('images/Default.jpg');
                                $isDefaultImage = basename($imageUrl) === 'Default.jpg';
                            @endphp
                            {{-- <span id="imageName">
                                {{ $imageExists ? basename($teams->image) : 'No image selected' }}
                            </span> --}}
                              <span id="imageName">
                                {{ !$isDefaultImage && $imageExists ? basename($teams->image) : 'No image' }}
                            </span>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="role" name="role"  value="{{ $teams->role }}"  required>
                    </div>
{{--                     
                    <div id="social-container" class ="social-fields">
                        <div class="" id="social-1"  style="border: #2C9DD4 1px solid; padding: 10px; border-radius: 5px;">
                            <button class="plus-btn float-end mb-2" title="Update New social Media" onclick="addSocialFields()">+</button>
                            <div class="form-group">
    ...
    <script>
        ...
        // Function to Update new social fields
        function addSocialFields() {
            const socialContainer = document.getElementById('social-container');
            const newDiv = document.createElement('div');
            newDiv.classList.Update('social-fields');
        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
           ...
           fetch('{{ route('api.teams.update', $teams->id) }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
                ...
                .then(data => {
                    Swal.close();
                    if (data.success) {
                        notify('success', 'Teams Member Details Updated Successfully!');
                        setTimeout(function() {
                            window.location.href = '{{ route('teams.index') }}';
                        }, 1500);

                    } else {
                        notify('error', data.message || 'There was an error updating the teams Section Detail page.');
                    }
                })
                .catch(error => {
                    Swal.close();
        ...
        if (deleteImageBtn) {
            deleteImageBtn.addEventListener('click', function () {
                window.deleteImageWithConfirm({
                    url: '{{ route('api.teams.deleteImage', $teams->id) }}',
