...
{{-- <div class="container-fluid" style="width: 101%; padding-right: 7px; padding-left: 0px; margin-right: auto; margin-left: auto; margin-top: 9px;">
    <div class="card" style="background-color: #f8f9fa; border-color: #e3e6f0;">
        <div class="card-header" style="background-color: #a9b5df; color: white; padding: 10px 15px 1px;">
            <h2 class="user-listing-header" style="text-align: center; font-size: 1.5em; margin-bottom: 6px; font-weight: bold; color: #2d336b;">Add FAQ Category</h2>
        </div> --}}
        ...
        <div class="container">
            <nav aria-label="breadcrumb-nav">
                <ol class="breadcrumb breadcrumb-style-2 my-20">
                    ...
                    <li class="breadcrumb-item breadcrumb-item-style-2 active" aria-current="page">Add FAQ Category</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="card" >
        <div class="card-header">
            <h4 class="user-listing-header">Add FAQ Category</h4>
        </div>
        <div class="card-body">
...
<script>
    document.getElementById('submitBtn').addEventListener('click', function() {
        ...
        fetch('{{ route('api.faq_categories.store') }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            Swal.close();
            if (data.success) {
                notify('success', 'FAQ Category created Successfully!');
                setTimeout(function() {
                    window.location.href = '{{ route('faq_categories.index') }}';
                }, 1500);
            } else {
                notify('error', 'There was an error creating the FAQ category.');
            }
        })
        .catch(error => {
            Swal.close();
