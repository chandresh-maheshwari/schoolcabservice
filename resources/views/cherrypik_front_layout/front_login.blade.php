...
<head>
    ...
    <title>@yield('title', 'Login | ટહુકાર - Tahukar.in')</title>
...
<body>
    ...
    <div class="container">
        ...
        <p class="welcome-message"></p>
            <!-- <p class="signin-message">Sign in to continue.</p> -->
            ...
            <label>
                <input id="email" type="email" placeholder="" class="input" name="email" required autocomplete="email" autofocus>
                <span>Email</span>
                <div id="emailError" class="text-danger"></div>
            </label>
            <label>
                <input id="password" type="password" placeholder="" class="input" name="password" required autocomplete="current-password">
                <span>Password</span>
                <i id="togglePassword" class="fas fa-eye" style="cursor: pointer; position: absolute; right: 14px; top: 20px;"></i>
                <div id="passwordError" class="text-danger"></div>
            </label>
            <button type="submit" class="submit">Submit</button>
            <p class="signin">Not a member? <a href="{{ route('front.register') }}">Sign Up</a></p>
            <p class="forgot-password" style="text-align: center;">
       <a href="{{ route('forgot-password', ['source' => 'front']) }}" style="color: #2d336b;">Forgot Password?</a>
   </p>        </form>
    </div>
    @endsection
        ...
        document.getElementById('loginForm').addEventListener('submit', async function(event) {
            ...
            try {
                const response = await fetch('{{ route('api.login') }}', {
                    ...
                    body: JSON.stringify({ email, password, source: 'front' })
                });
    ...
    <style>
        .welcome-message, .signin-message {
    text-align: center;
    margin: -8px ;
    font-size: 1.5em;
    font-weight: bold;
