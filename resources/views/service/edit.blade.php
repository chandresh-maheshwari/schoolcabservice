...
@section('content')
            ...
            <div class="container">
                <nav aria-label="breadcrumb-nav">
                    <ol class="breadcrumb breadcrumb-style-2 my-20">
                        ...
                        <li class="breadcrumb-item"><a class="breadcrumbLink" href="{{ route('service.index') }}">Service</a>
                        <li class="breadcrumb-item breadcrumb-item-style-2 active" aria-current="page">Edit Services Deatils
                        </li>
                    </ol>
                </nav>
            </div>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                <h4 class="about-us-create-header">Edit Service</h4>
            </div>
            <div class="card-body">
                <form id="heroSectionForm" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="service_icon" style="font-weight: bold;">Service Icon</label>
                        <div class="input-group" style="max-width: 400px;">
                            ...
                            <input type="text" class="form-control" id="service_icon" name="service_icon"
                                value="{{ $service->service_icon }}" required placeholder="Select an icon..."
                    ...
                    {{-- <div class="form-group">
                        ...
                        <div class="mt-2">
                            ...
                            <span id="imageName">{{ $service->image ? basename($service->image) : 'No image selected' }}</span>
                            <img id="imagePreview" src="{{ $service->image ? asset($service->image) : '' }}" alt="Image Preview"
                                style="display: {{ $service->image ? 'block' : 'none' }}; width: 100px; height: 100px; margin-top: 10px;">
                            @if ($service->image)
                    ...
                    <div class="form-group">
                        ...
                        <div class="mt-2">
                            ...
                            <button type="button" class="btn btn-primary" id="uploadImageBtn"
                                ...
                                style="background-color: #2C9DD4; color: white;">Upload Image</button>
                            @php
                                $imagePath = $service->image ? public_path($service->image) : null;
                                $imageExists = $imagePath && File::exists($imagePath);
                                $imageUrl = $imageExists ? asset($service->image) : asset('images/Default.jpg');
                                $isDefaultImage = basename($imageUrl) === 'Default.jpg';
                            @endphp
                            <span id="imageName">
                                {{ !$isDefaultImage && $imageExists ? basename($service->image) : 'No image' }}
                            </span>
                        </div>
                    ...
                    <a href="{{ route('service.index') }}" class="btn btn-secondary" id="cancelBtn">Cancel</a>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
            ...
            if (!formData.get('service_icon')) {
                $('#service_icon').closest('.form-group').append(
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        notify('error', data.message ||
                            'There was an error updating the service Section Detail page.');
                    }
                })
                .catch(error => {
                    Swal.close();
        ...
        // check if default image is shown
        // var isDefaultImage = currentImageSrc.includes('Default.jpg');
        // if (!imageInput.files.length && isDefaultImage) {
        ...
        $(document).ready(function() {
            ...
            $('[role="iconpicker"]').on('iconpickerSelected', function(e) {
                $('#service_icon').val(e.iconpickerValue);
                $('#service_icon')[0].dispatchEvent(new Event('input'));
            ...
            updateIconPreview($('#service_icon').val());
        ...
        if (deleteImageBtn) {
            deleteImageBtn.addEventListener('click', function() {
                window.deleteImageWithConfirm({
                    url: '{{ route('api.service.deleteImage', $service->id) }}',
