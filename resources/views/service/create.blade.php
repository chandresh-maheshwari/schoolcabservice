            ...
            <div class="container">
                <nav aria-label="breadcrumb-nav">
                    <ol class="breadcrumb breadcrumb-style-2 my-20">
                        ...
                        <li class="breadcrumb-item breadcrumb-item-style-2 active" aria-current="page">Add Service</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                <h4 class="about-us-create-header">Add Services Details</h4>
            </div>
            <div class="card-body">
                <form id="heroSectionForm" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="service_icon" style="font-weight: bold;">Service Icon<span
                                style="color: red;">*</span></label>
                        <div class="input-group" style="max-width: 400px;">
                            ...
                            <button type="button" class="btn btn-outline-secondary" role="iconpicker"
                                data-iconset="fontawesome5" data-input="service_icon" data-preview="stat-icon-preview-1"
        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
            ...
            if (!formData.get('image') || !formData.get('image').name) {
                $('#uploadImageBtn').after(
                    '<span class="error-message" style="color: red;">Client Image is required.</span>');
                isValid = false;
            }
            if (!isValid) {
                return;
            ...
            fetch('{{ route('api.service.store') }}', {
                ...
                .then(data => {
                    Swal.close();
                    if (data.success) {
                        notify('success', 'Service Details Created Successfully!');
                        setTimeout(function() {
                            window.location.href = '{{ route('service.index') }}';
                        }, 1500);
                    } else {
                        notify('error', data.message || 'There was an error creating the service details.');
                    }
                })
                .catch(error => {
                    Swal.close();
