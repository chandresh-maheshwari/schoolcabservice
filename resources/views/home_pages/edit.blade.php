...
{{-- <div class="container-fluid" style="width: 101%; padding-right: 7px; padding-left: 0px; margin-right: auto; margin-left: auto; margin-top: 9px;">
    <div class="card" style="background-color: #f8f9fa; border-color: #e3e6f0;">
        <div class="card-header" style="background-color: #a9b5df; color: white; padding: 10px 15px 1px;">
            <h2 class="home-page-edit-header" style="text-align: center; font-size: 1.5em; margin-bottom: 6px; font-weight: bold; color: #2d336b;">Edit Home Page</h2>
        </div> --}}
        ...
        <div class="container">
            <nav aria-label="breadcrumb-nav">
                <ol class="breadcrumb breadcrumb-style-2 my-20">
                    ...
                    <li class="breadcrumb-item"><a class="breadcrumbLink" href="{{ route('home_pages.index') }}">Home Pages</a></li>
                    <li class="breadcrumb-item breadcrumb-item-style-2 active" aria-current="page">Edit Home Page</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h4 class="home-page-edit-header">Edit Home Page</h4>
                </div>
        <div class="card-body">
            <form id="homePageForm" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="title" name="title" value="{{ $homePage->title }}" required>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control" id="template" name="template">
                        ...
                        @foreach(($templates ?? []) as $tpl)
                            <option value="{{ $tpl }}" {{ $homePage->template === $tpl ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ', $tpl)) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control" id="category_id" name="category_id" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ $homePage->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <button type="button" class="btn btn-primary" id="uploadImageBtn" onclick="document.getElementById('image').click();" style="background-color: #2C9DD4; color: white;">Upload Image</button>
                    <span id="imageName">{{ $homePage->image ? basename($homePage->image) : 'No image selected' }}</span>
                    <img id="imagePreview" src="{{ asset('storage/' . $homePage->image) }}" alt="Image Preview" style="display: block; width: 100px; height: 100px; margin-top: 10px;">
                    </div>
                </div>
                <div class="form-group">
                    <label for="description" style="font-weight: bold;">Description <span style="color: red;">*</span></label>
                    <textarea class="form-control" id="description" name="description" rows="4" required>{{ $homePage->description }}</textarea>
                </div>
...
<script>
    ...
    document.getElementById('submitBtn').addEventListener('click', function() {
        ...
        if (!formData.get('category_id')) {
            $('#category_id').parent().find('.error-message').remove();
            $('#category_id').parent().append('<span class="error-message" style="color: red;">Category is required.</span>');
            isValid = false;
        }
        ...
        if (!formData.get('image').name && !document.getElementById('imageName').textContent.trim()) {
        ...
        fetch('{{ route('api.home_pages.update', $homePage->id) }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            Swal.close();
            if (data.success) {
                notify('success', 'Home Page updated Successfully!');
                setTimeout(function() {
                    window.location.href = '{{ route('home_pages.index') }}';
                }, 1500);
            } else {
                notify('error', 'There was an error updating the Home Page.');
            }
        })
        .catch(error => {
            Swal.close();
