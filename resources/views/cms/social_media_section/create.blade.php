...
@section('content')
            ...
            <div class="card-body">
                <form id="socialForm">
                    ...
                    <div class="form-group">
                        <label for="social_name" style="font-weight: bold;">Social Name <span
                    ...
                    <button type="button" class="btn btn-primary" id="submitBtn"
                        style="background-color: #2C9DD4; color: white;">Create</button>
    ...
    <script>
        ...
        $(document).on('input', '#social_link', function() {
            let urlPattern = /^(https?:\/\/)[^\s/$.?#].[^\s]*$/i;

            if (!urlPattern.test(this.value)) {
                this.setCustomValidity("Please enter a valid URL (https://...)");
            } else {
        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
            ...
            fetch('{{ route('api.socialMediaSection.store') }}', {
                ...
                .then(response => {
                    if (!response.ok) {
                        return response.json().then(err => {
                            throw err; // 👈 validation errors yahin se aayenge
                        });
                    }
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        notify('success', data.message || 'Social created Successfully!');
                        setTimeout(function() {
