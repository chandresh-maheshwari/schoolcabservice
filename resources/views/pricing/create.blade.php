        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
            ...
            fetch('{{ route('api.pricing.store') }}', {
                ...
                .then(data => {
                    Swal.close();
                    if (data.success) {
                        notify('success', 'pricing details created Successfully!');
                        setTimeout(function() {
                            window.location.href = '{{ route('pricing.index') }}';
                        }, 1500);
                    } else {
                        notify('error', data.message || 'There was an error creating the pricing details.');
                    }
                })
                .catch(error => {
                    Swal.close();
